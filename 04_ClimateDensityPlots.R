...
climate.density.plots <- function(data.Yn, data.means ){
  ...
  p.Tmin_Spring <- ggplot(data.Yn, aes(x=Tmin_Spring, colour=WPBR_stat)) +
    ...
    theme(legend.position="none", panel.background = element_rect(fill='transparent') , text = element_text(size = 20)) + scale_color_manual(values = c("black", "cyan4")) + 
    xlim(-25,0) + ylab("Density")
  
  p.Tmin_Summer <- ggplot(data.Yn, aes(x=Tmin_Summer, color=WPBR_stat)) +
    geom_density(aes( y = ..scaled..)) + 
    geom_vline(data=data.means, aes(xintercept= Tmin_Summer, group=WPBR_stat),  colour = c("black", "cyan4"),
                                linetype="dashed") + 
    theme(legend.position="none", panel.background = element_rect(fill='transparent'), text = element_text(size = 20)) +
    scale_color_manual(values = c("black", "cyan4"))+  xlim(-25,0) +  ylab("Density")
  
  p.Tmin_Fall <-ggplot(data.Yn, aes(x=Tmin_Fall, color=WPBR_stat)) +
    geom_density(aes( y = ..scaled..)) + 
    geom_vline(data=data.means, aes(xintercept= Tmin_Fall, group=WPBR_stat),  colour = c("black", "cyan4"),
                                linetype="dashed") + 
    theme(legend.position="none", panel.background = element_rect(fill='transparent') , text = element_text(size = 20))+
    scale_color_manual(values = c("black", "cyan4"))+ xlim(-25,0) + ylab("Density")
  ...
  p.Tmax_Spring <-ggplot(data.Yn, aes(x=Tmax_Spring, color=WPBR_stat)) +
    geom_density(aes( y = ..scaled..)) + 
    geom_vline(data=data.means, aes(xintercept= Tmax_Spring,  group=WPBR_stat),  colour = c("black", "cyan4"),linetype="dashed") + 
    theme(legend.position="none", panel.background = element_rect(fill='transparent') , text = element_text(size = 20)) +
    scale_color_manual(values = c("black", "cyan4")) + 
    xlim(20,30) + ylab("Density")
  
  p.Tmax_Summer <-ggplot(data.Yn, aes(x=Tmax_Summer, color=WPBR_stat)) +
    geom_density(aes( y = ..scaled..)) + 
    geom_vline(data=data.means, aes(xintercept= Tmax_Summer,  group=WPBR_stat),  colour = c("black", "cyan4"),linetype="dashed")+ 
    theme(legend.position="none", panel.background = element_rect(fill='transparent'), text = element_text(size = 20)) +
  ...
  p.Tmax_Fall <- ggplot(data.Yn, aes(x=Tmax_Fall, color=WPBR_stat)) +
    geom_density(aes( y = ..scaled..)) + 
    geom_vline(data=data.means, aes(xintercept= Tmax_Fall, color=WPBR_stat),
                                linetype="dashed")+ theme(legend.position="none", panel.background = element_rect(fill='transparent'), text = element_text(size = 25))+
    scale_color_manual(values = c("black", "cyan4"))+ xlim(20,30) + ylab("Density")
  
  # PRCP
  p.PRCP_Spring <-ggplot(data.Yn, aes(x=PRCP_Spring, color=WPBR_stat)) +
    geom_density(aes( y = ..scaled..)) + geom_vline(data=data.means, aes(xintercept= PRCP_Spring,  group=WPBR_stat),  colour = c("black", "cyan4"), linetype="dashed") + 
    scale_color_manual(values = c("black", "cyan4")) +
    theme(panel.background = element_rect(fill='transparent'), text = element_text(size = 20)) + xlim(0,300) + ylab("Density") + labs(name='')
  
  p.PRCP_Summer <-ggplot(data.Yn, aes(x=PRCP_Summer, color=WPBR_stat)) +
    geom_density(aes( y = ..scaled..)) + 
    geom_vline(data=data.means, aes(xintercept= PRCP_Summer,  group=WPBR_stat),  colour = c("black", "cyan4"), linetype="dashed")+ 
    theme(legend.position="none", panel.background = element_rect(fill='transparent'), text = element_text(size = 20))+
    scale_color_manual(values = c("black", "cyan4"))+ xlim(0,300) + ylab("Density") 
  ...
  p.VPD_Spring <-ggplot(data.Yn, aes(x=VPD_Spring, color=WPBR_stat)) +
    geom_density(aes( y = ..scaled..)) + 
    geom_vline(data=data.means, aes(xintercept= VPD_Spring,  group=WPBR_stat),  colour = c("black", "cyan4"),
                                linetype="dashed")+ 
    theme(legend.position="none", panel.background = element_rect(fill='transparent'), text = element_text(size = 20))+
    scale_color_manual(values = c("black", "cyan4"))+ xlim(200,1500)  + ylab("Density")
  ...
  p.VPD_Fall <-ggplot(data.Yn, aes(x=VPD_Fall, color=WPBR_stat)) +
    geom_density(aes( y = ..scaled..)) + 
    geom_vline(data=data.means, aes(xintercept= VPD_Fall,  group=WPBR_stat),  colour = c("black", "cyan4"),
                                linetype="dashed")+ 
    theme(legend.position="none", panel.background = element_rect(fill='transparent'), text = element_text(size = 20))+
    scale_color_manual(values = c("black", "cyan4"))+ xlim(200,1500)  + ylab("Density")
  ...
  plot<- ggarrange(
    ...
    labels= c( "a.", "b.", "c.", 
               "d.", "e.", "f.", 
               "g.", "h.", "i.",
               "j.", "k.", "l."), font.label = list(size = 20), common.legend = TRUE)
  return(plot)
}
