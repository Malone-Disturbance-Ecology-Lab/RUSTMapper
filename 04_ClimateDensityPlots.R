...
# Functions:
data.Yn <- data.5years %>% re_process
...
data.means <- seasonal.means(data.Yn)
...
climate.density.plots <- function(data.Yn, data.means ){
  ...
  p.Tmin_Spring <- ggplot(data.Yn, aes(x=Tmin_Spring, colour=WPBR_stat)) +
    geom_density(aes( y = ..scaled..)) + geom_vline(data=data.means, aes(xintercept= Tmin_Spring, group=WPBR_stat), linetype="dashed",  colour = c("black", "cyan4") ) + 
    theme(legend.position="none", panel.background = element_rect(fill='transparent')) +
    scale_color_manual(values = c("black", "cyan4")) + xlim(-25,0)
  ...
  p.Tmin_Summer <-ggplot(data.Yn, aes(x=Tmin_Summer, color=WPBR_stat)) +
    geom_density(aes( y = ..scaled..)) + geom_vline(data=data.means, aes(xintercept= Tmin_Summer, group=WPBR_stat),  colour = c("black", "cyan4"),
                                linetype="dashed") + theme(legend.position="none", panel.background = element_rect(fill='transparent')) +
    scale_color_manual(values = c("black", "cyan4"))+  xlim(-25,0)
  ...
  p.Tmax_Spring <-ggplot(data.Yn, aes(x=Tmax_Spring, color=WPBR_stat)) +
    geom_density(aes( y = ..scaled..)) + geom_vline(data=data.means, aes(xintercept= Tmax_Spring,  group=WPBR_stat),  colour = c("black", "cyan4"),linetype="dashed") + theme(legend.position="none", panel.background = element_rect(fill='transparent'))+
    scale_color_manual(values = c("black", "cyan4")) + xlim(20,30)
  ...
  p.PRCP_Spring <-ggplot(data.Yn, aes(x=PRCP_Spring, color=WPBR_stat)) +
    ...
    theme(panel.background = element_rect(fill='transparent')) + xlim(0,300)
  ...
  p.PRCP_Summer <-ggplot(data.Yn, aes(x=PRCP_Summer, color=WPBR_stat)) +
    geom_density(aes( y = ..scaled..)) + geom_vline(data=data.means, aes(xintercept= PRCP_Summer,  group=WPBR_stat),  colour = c("black", "cyan4"),
                                linetype="dashed")+ theme(legend.position="none", panel.background = element_rect(fill='transparent'))+
    scale_color_manual(values = c("black", "cyan4"))+ xlim(0,300)
  ...
  p.VPD_Spring <-ggplot(data.Yn, aes(x=VPD_Spring, color=WPBR_stat)) +
    geom_density(aes( y = ..scaled..)) + geom_vline(data=data.means, aes(xintercept= VPD_Spring,  group=WPBR_stat),  colour = c("black", "cyan4"),
                                linetype="dashed")+ theme(legend.position="none", panel.background = element_rect(fill='transparent'))+
    scale_color_manual(values = c("black", "cyan4"))+ xlim(200,1500)
...
png("04_Figure_ClimateDensity_5Years", width = 900, height = 800)
plots.5
dev.off()

png("04_Figure_ClimateDensity_10Years", width = 900, height = 800)
plots.10
dev.off()

png("04_Figure_ClimateDensity_20Years", width = 900, height = 800)
plots.20
dev.off()
