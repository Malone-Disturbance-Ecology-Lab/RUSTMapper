...
# Invading
names(invading)
m.invading.5 <- VSURF(invading[, 2:26], invading[, 1], ntree = 1000,
                      RFimplem = "randomForest", 
                      clusterType = "PSOCK", 
                      verbose = TRUE,
                      ncores = detectCores() - 2, parallel= TRUE)

vars.m.invading.5 <- names( invading[, 2:26]) [m.invading.5$varselect.pred]

# Established
# Fit vars in two groups climate and site
m.established.5 <- VSURF(established[, 2:26], established[, 1], ntree = 1000,
                         RFimplem = "randomForest", 
                         clusterType = "PSOCK", 
                         verbose = TRUE,
                         ncores = detectCores() - 2, parallel= TRUE)

vars.m.established.5 <-names( established[, c(2:26)]) [m.established.5$varselect.pred]
...
save(m.all.5n.site, vars.m.all.5, 
     ...
     file = "WPBR_Variable_Selction_5_GRIDMET.RData")
...
data.10n <- data.10 %>% select(vars.m.all.10n) %>% filter(RUST < 2) %>% na.omit()
...
# All
m.all.10n.site <- VSURF(data.10n[, 2:26], data.10n[, 1], ntree = 1000,
                       RFimplem = "randomForest", 
                       clusterType = "PSOCK", 
                       verbose = TRUE,
                       ncores = detectCores() - 2, parallel= TRUE)

vars.m.all.10 <-unique(names( data.10n[, 2:26])[ m.all.10n.site$varselect.pred])

# Invading
# Fit vars in two groups climate and site

names(invading)
m.invading.10 <- VSURF(invading[, 2:26], invading[, 1], ntree = 1000,
                      RFimplem = "randomForest", 
                      clusterType = "PSOCK", 
                      verbose = TRUE,
                      ncores = detectCores() - 2, parallel= TRUE)

vars.m.invading.10 <- names( invading[, 2:26]) [m.invading.10$varselect.pred]

# Established
# Fit vars in two groups climate and site
m.established.10 <- VSURF(established[, 2:26], established[, 1], ntree = 1000,
                         RFimplem = "randomForest", 
                         clusterType = "PSOCK", 
                         verbose = TRUE,
                         ncores = detectCores() - 2, parallel= TRUE)

vars.m.established.10 <-names( established[, c(2:26)]) [m.established.10$varselect.pred]
...
save(m.all.10n.site, vars.m.all.10, 
     ...
     file = "WPBR_Variable_Selction_10_GRIDMET.RData")
###### 20 year model fit ####
...
data.20n <- data.20 %>% select(vars.m.all.20n) %>% filter(RUST < 2) %>% na.omit()
...
m.all.20n.site <- VSURF(data.20n[, 2:26], data.20n[, 1], ntree = 2000,
                       RFimplem = "randomForest", 
                       clusterType = "PSOCK", 
                       verbose = TRUE,
                       ncores = detectCores() - 2, parallel= TRUE)

vars.m.all.20 <-unique(names( data.20n[, 2:26])[ m.all.20n.site$varselect.pred])

# Invading
# Fit vars in two groups climate and site

names(invading)
m.invading.20 <- VSURF(invading[, 2:26], invading[, 1], ntree = 1000,
                      RFimplem = "randomForest", 
                      clusterType = "PSOCK", 
                      verbose = TRUE,
                      ncores = detectCores() - 2, parallel= TRUE)

vars.m.invading.20 <- names( invading[, 2:26]) [m.invading.20$varselect.pred]

# Established
# Fit vars in two groups climate and site
m.established.20 <- VSURF(established[, 2:26], established[, 1], ntree = 1000,
                         RFimplem = "randomForest", 
                         clusterType = "PSOCK", 
                         verbose = TRUE,
                         ncores = detectCores() - 2, parallel= TRUE)

vars.m.established.20 <-names( established[, c(2:26)]) [m.established.20$varselect.pred]
...
save(m.all.20n.site, vars.m.all.20, 
     ...
     file = "WPBR_Variable_Selction_20_GRIDMET.RData")
